...
# generate a matrix of every combination of 0 to 100
x <- seq(0,100,10)
...
access_thresold_Ai <- vector(mode = "list", length = 10)
access_thresold    <- vector(mode = "list", length = 10)
access_thresold_weighted <- vector(mode = "list", length = 10)
...
for (h in 1:nrow(services)){
 ...
 # beds <- services[h,]
  ...
  for(i in 1:10){
    access_thresold_weighted[[i]][h] <- sum(sapply(1:length(access),  function(x)
...
access_thresold_rez <- vector(mode = "list", length = 10)
...
final_Ai <- vector(mode = "list", length = 10)
...
for(h in 1:10){
  ...
  for(i in 1:nrow(dist)){
    ...
    for(j in 1:5){
      rez[j] <- best_services[j]/(Vj[j] * dist[i,j+1] )
    }
...
huaq@data$thresold8 <- final_Ai[[8]]
huaq@data$thresold9 <- final_Ai[[9]]
huaq@data$thresold10 <- final_Ai[[10]]
